...
typedef chess::board ChessBoard;
struct PositionHistory : public std::vector<chess::board>
{
    using std::vector<chess::board>::vector;
    const chess::board& Last() const
    {
        return back();
    }
    ...
    const chess::board& GetPositionAt(int idx) const
    {
        return operator[](idx);
    }
};
